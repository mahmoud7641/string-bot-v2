...
async def executor(client: Anony, message: Message):
    if len(message.command) < 2:
        return await edit_or_reply(message, text="<b>ماذا تريد ان تفعل ؟</b>")
    try:
    ...
    if "\n" in text:
        ...
        for x in code:
            ...
            try:
                ...
                await edit_or_reply(message, text=f"<b>خطأ :</b>\n<pre>{err}</pre>")
            output += f"<b>{code}</b>\n"
